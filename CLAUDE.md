...
**목표:** 기능 개발·스키마 변경·시드 데이터 생성을 에이전트 팀이 설계→구현→검증 파이프라인으로 처리한다.

**트리거:** 코드 작업(기능 추가/수정/버그 수정) 요청 시 `feature-build` 스킬을 사용하라. DB 구조 변경은 `schema-change`, 시드 코스 추가는 `seed-data`. 단순 질문·코드 설명은 직접 응답 가능.
